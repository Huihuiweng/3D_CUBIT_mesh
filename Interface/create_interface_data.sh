...
# Read GRID-format data
  grd2xyz ${Int_input_data} | gawk '{if($1>= '"${X1}"' && $1<= '"${X2}"' && $2>= '"${Y1}"' && $2<='"${Y2}"') print ($1-('"$Lon_ref"'))*'"$Lon_scale"'*1e3, ($2-('"$Lat_ref"'))*'"$Lat_scale"'*1e3, -$3*1e3}' >  tempfile/output.xyz
else
...
# For other slip model, one needs to change the number of head line (it's 9 lines here), and sort the order of columns in this sequence: longitude, latitude, depth.
  ...
  #blockmedian ${Int_input_data}  -R${X1}/${X2}/${Y1}/${Y2} -I${Int_sample_inc} | sort -k1,1n -k2,2n > tempfile/output_mean.xyz
  surface tempfile/output_mean.xyz  -Gtempfile/output_surf.grd -R  -I${Int_sample_inc} -T1
  grd2xyz tempfile/output_surf.grd | gawk '{print ($1-('"$Lon_ref"'))*'"$Lon_scale"'*1e3, ($2-('"$Lat_ref"'))*'"$Lat_scale"'*1e3, -$3*1e3}' >  tempfile/output.xyz
fi
...
gawk '{print $1/1e3/'"$Lon_scale"'+('"$Lon_ref"'),$2/1e3/'"$Lat_scale"'+('"$Lat_ref"'),$3/-1e3}' tempfile/${Subxyz}  | xyz2grd -R -I${final_Int_sample_inc} -Gtempfile/${Subxyz}.grd

...
if [ $Sur_GRD_data = "1" ] ; then
    ...
    if [ ! -f tempfile/cutted_${X1}_${X2}_${Y1}_${Y2}.grd ] ; then
       ...
       grdcut $Sur_input_data -Gtempfile/cutted_${X1}_${X2}_${Y1}_${Y2}.grd -R${X1}/${X2}/${Y1}/${Y2}
    fi
    grd2xyz tempfile/cutted_${X1}_${X2}_${Y1}_${Y2}.grd | gawk '{print ($1-('"$Lon_ref"'))*'"$Lon_scale"'*1e3, ($2-('"$Lat_ref"'))*'"$Lat_scale"'*1e3, $3}' >  tempfile/topo.xyz
else
    gawk '{if($1>= '"${X1}"' && $1<= '"${X2}"' && $2>= '"${Y1}"' && $2<='"${Y2}"') print ($1-('"$Lon_ref"'))*'"$Lon_scale"'*1e3, ($2-('"$Lat_ref"'))*'"$Lat_scale"'*1e3, $3}' ${input_data} > tempfile/topo.xyz
fi
...
### Original data
grdimage   tempfile/cutted_${X1}_${X2}_${Y1}_${Y2}.grd   -R${X1}/${X2}/${Y1}/${Y2} -JX4i -B0.5f0.1/0.5f0.1:."Original topography":WSne -Ctemp.cpt -K -Q > ps/${Sur_sigma}_${Sur_inc}_topography.eps
grdcontour tempfile/cutted_${X1}_${X2}_${Y1}_${Y2}.grd   -R -J -C500 -A- -O -K >> ps/${Sur_sigma}_${Sur_inc}_topography.eps
### Smoothed data
...
pscontour tempfile/${Subxyz}  -R${LowerX}/${UpperX}/${LowerY}/${UpperY} -B50000f10000:."Smoothed and resampled topography":WSne -J -Ctemp.cpt -I -A- -O -K -X5i  >> ps/${Sur_sigma}_${Sur_inc}_topography.eps

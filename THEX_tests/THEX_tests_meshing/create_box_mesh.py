...
vp  = 4330     # P wave speed (m/s)
vs  = 2500     # S wave speed (m/s)
rho = 2243     # density (g/m^3)
Q   = 13
...
if(Interface and Topography):
    output_mesh    = mesh_name + "_curvedfault_curvedtopo"
elif(not Interface and Topography):
    output_mesh    = mesh_name + "_p" + "_s_" + str(Strike) + "_d_" + str(Dip) + "_d_" + str(Dep) + "_t"
elif(Interface and not Topography):
    output_mesh    = mesh_name + "_curvedfault_planarsur"
else:
    output_mesh    = mesh_name + "_p" + "_s_" + str(Strike) + "_d_" + str(Dip) + "_d_" + str(Dep) + "_p"
...
bottom = numpy.asarray(list_surf)[numpy.argwhere(center_depth == numpy.amin(center_depth))[:,0]].tolist()
topo   = numpy.asarray(list_surf)[numpy.argwhere(center_depth == numpy.amax(center_depth))[:,0]].tolist()
#bottom = [surface_list]
#topo   = [surface_list]

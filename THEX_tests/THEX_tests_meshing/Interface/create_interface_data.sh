...
#  gawk '{if(NR>9 && $15==1) print $4,$3,$5}' ${Int_input_data} | blockmedian  -R${X1}/${X2}/${Y1}/${Y2} -I${Int_sample_inc} | sort -k1,1n -k2,2n > temp_file/output_mean.xyz
  gawk 'BEGIN{FS=","}{if(NR>1) print $1-633.63800, $2-4931.28500,$3}' ${Int_input_data} | gawk '{print $1*cos(-45/180*3.14)+$2*sin(-45/180*3.14),-$1*sin(-45/180*3.14)+$2*cos(-45/180*3.14),$3}' | blockmedian  -R-2/2/-15/15  -I${Int_sample_inc} | sort -k1,1n -k2,2n > temp_file/output_mean.xyz
  surface temp_file/output_mean.xyz  -Gtemp_file/output_surf.grd -R  -I${Int_sample_inc} -T1
  grd2xyz temp_file/output_surf.grd | gawk '{print $1*1e3, $2*1e3, $3*1e3}'>  temp_file/output.xyz
...
makecpt -Ctopo  -T-5/0/0.01 -D -N > temp.cpt
...
gawk 'BEGIN{FS=","}{if(NR>1) print $1-633.63800, $2-4931.28500,$3}' ${Int_input_data} | gawk '{print $1*cos(-45/180*3.14)+$2*sin(-45/180*3.14),-$1*sin(-45/180*3.14)+$2*cos(-45/180*3.14),$3}' | pscontour -R -JX4i -B0.5f0.1:."Color Original; Lines Smoothed":WSne -I -Ctemp.cpt  -K > ps/${Int_sigma}_${Int_inc}_interface.eps
...
gawk '{print $1/1e3,$2/1e3,$3/1e3}' temp_file/${Subxyz}  | xyz2grd -R -I${final_Int_sample_inc} -Gtemp_file/${Subxyz}.grd
